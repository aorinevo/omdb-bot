  ...
  register: (server, options) => {
    server.route({
      ...
      handler: async (request, h) => {
        ...
        let findPoster
        try {
          findMovie = JSON.parse(await movieCall(process.env.API_KEY, request.params.title));
          const movieId = findMovie.imdbID;

          findPoster = await posterCall(process.env.API_KEY, movieId)
        } catch (err) {
          console.error(err)
        }
