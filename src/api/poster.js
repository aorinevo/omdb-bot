...
let posterCall = async (api, title) => {
  const { moviereq, movieres, payload: moviePayload } = await Wreck.get(
    `http://www.omdbapi.com/?apikey=${api}&t=${title}`
  );
  const { req, res, payload } = await Wreck.get(
    `http://img.omdbapi.com/?apikey=${api}&i=${moviePayload.imdbID}`
  );
  return payload;
};
